...
- 글을 저장하면 그 글의 마크다운 파일을 git에 커밋하고 GitHub에 push할 수 있다 — Gemini API가 이 저장소의 커밋 컨벤션에 맞는 메시지를 추천해주고, 관리자가 확인/수정한 뒤 직접 "커밋 & 푸시" 버튼을 눌러야 실행된다 (자동 실행 아님).
...
- 글 저장에 성공하면 git 커밋 메시지 추천이 담긴 패널이 뜬다. "커밋 & 푸시"를 누르면 그 글의 `.md` 파일만 커밋되어 GitHub에 올라가고, "건너뛰기"를 누르면 그냥 글 상세 페이지로 이동한다 (About 페이지는 이 기능 대상이 아니라 저장 즉시 이동한다)
