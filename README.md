...
- 방문자는 로그인 없이 모든 글을 열람·검색할 수 있고, 상단 메뉴에서 "전체 글 보기" / "About me"로 이동하거나 글 목록 페이지의 카테고리별 보기·정렬(이름/날짜/수정순 × 오름/내림차순) 드롭다운으로 원하는 순서로 볼 수 있다. 글이 없는 카테고리는 목록에서 자동으로 숨겨진다. 검색은 부분 문자열 매칭이라 한글 검색어를 짧게 입력해도(예: "테스") 잘 찾는다.
- 전체 글 목록과 "이력 조회" 화면은 10/25/50개씩 페이지를 나눠서 볼 수 있다.
- 글이나 About 본문에 mermaid 코드블록(\`\`\`mermaid)을 쓰면 다이어그램으로 렌더링된다.
...
- "Git 관리" 화면에서 아직 커밋되지 않은 글/About 변경 사항을 목록으로 보고, 원하는 항목만 골라 커밋 메시지를 추천받은 뒤 GitHub에 push할 수 있다 — Gemini API가 이 저장소의 커밋 컨벤션에 맞는 메시지를 추천해주고, 관리자가 확인/수정한 뒤 직접 버튼을 눌러야 실행된다 (자동 실행 아님).
- 로그인/로그아웃, 글 작성·수정·삭제, git 커밋&푸시까지 전부 "이력 조회" 한 화면에서 볼 수 있다.
...
- 검색: MariaDB `LIKE` 부분 문자열 매칭 (한글은 FULLTEXT 인덱스로 짧은 검색어를 못 찾는 문제가 있어 이 방식을 쓴다)
- 프론트엔드: 순수 HTML + jQuery + Vanilla JS, `marked.js` + `DOMPurify`(렌더링/새니타이즈), highlight.js(코드 하이라이팅), mermaid.js(다이어그램) — 전부 CDN이 아니라 `/vendor` 경로로 자체 호스팅. Toast UI Editor만 CDN 사용 (자체 호스팅이 기술적으로 불가능 — `CLAUDE.md` 참고)
- 관리자 편의 기능: Google Gemini API(무료 티어)로 git 커밋 메시지 추천
...
GEMINI_API_KEY=<Google AI Studio에서 발급한 무료 API 키 (선택 — 없으면 커밋 메시지 추천 기능만 비활성화)>
...
- 글쓰기 화면의 카테고리 선택 드롭다운에서 "+ 새 카테고리 만들기"를 고르면 그 자리에 slug/이름 입력창이 바로 뜬다 (팝업 아님). 기존 카테고리를 선택한 상태에서는 "카테고리 삭제" 버튼이 나타나며, 글이 남아있는 카테고리는 서버가 삭제를 막는다. 삭제가 성공하면 `posts/{slug}/` 폴더도 함께 지워진다.
- 상단 네비게이션의 "Git 관리"(`/admin/git`)에서 아직 커밋되지 않은 글/About 변경 사항을 전부 볼 수 있다. 커밋할 항목을 체크하고 "메시지 추천받기"로 Gemini의 추천을 받은 뒤(직접 수정 가능) "선택 항목 커밋 & 푸시"를 누르면 그 항목들만 하나의 커밋으로 GitHub에 올라간다.
- 이력 조회(로그인/로그아웃 + 글 작성/수정/삭제 + git 커밋/푸시), Git 관리, 계정 관리는 상단 네비게이션에 항상 링크로 노출된다 (로그인 상태일 때만)
...
- 관리자 화면의 "커밋 & 푸시" 기능을 쓰려면, 앱을 실행하는 OS 계정(`kam`)이 `origin`에 push할 수 있는 git 인증정보를 갖고 있어야 한다 (`credential.helper store` + `~/.git-credentials`에 GitHub PAT). 새 호스트에 배포할 때 이 설정이 없으면 커밋까지는 되고 push만 실패한다.
...
자세한 아키텍처/설계 원칙은 `CLAUDE.md`를 참고.
