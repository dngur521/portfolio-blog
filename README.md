...
- 프론트엔드: 순수 HTML + jQuery + Vanilla JS, `marked.js` + `DOMPurify`(렌더링/새니타이즈), highlight.js(코드 하이라이팅) — 전부 CDN이 아니라 `/vendor` 경로로 자체 호스팅. Toast UI Editor만 CDN 사용 (자체 호스팅이 기술적으로 불가능 — `CLAUDE.md` 참고)
