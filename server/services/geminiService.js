...
function buildPrompt({ title, content, changeType }) {
  const changeLabel = CHANGE_LABELS[changeType] || '글 변경';
  ...
  return [
    ...
    `이번 변경은 마크다운 블로그 글의 "${changeLabel}"이다.`,
    `글 제목: ${title}`,
    '글 내용(마크다운, 일부):',
    String(content || '').slice(0, 4000),
    '',
    '위 컨벤션에 맞는 커밋 메시지를 딱 한 줄, 한국어로 추천해라.',
    '이모지 하나 + 공백 + 종류(feat/docs 중 이 변경에 더 알맞은 것) + ": " + 간결한 요약 형태로만 답하고,',
    '다른 설명, 따옴표, 코드블록 없이 그 한 줄만 출력해라.',
  ].join('\n');
}

async function suggestCommitMessage({ title, content, changeType }) {
  if (!env.geminiApiKey) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      contents: [{ parts: [{ text: buildPrompt({ title, content, changeType }) }] }],
    }),
  });
